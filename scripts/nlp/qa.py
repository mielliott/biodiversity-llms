...
def query_openai(client: OpenAI, model: str, query: str, **kwargs) -> ChatCompletion:
    while (True):
        try:
...
def run_openai_model(model, header, max_tokens, num_responses, top_p, combine_responses, timeout, escape):
    ...
    for i, (input, question) in enumerate(questions):
        ...
        response = query_openai(client, model, question, n=num_responses, top_p=top_p, max_tokens=max_tokens, timeout=timeout)
