...
class Llama(Model):
    ...
    def run(self, queries: List[Tuple[str, str]]):
        ...
        all_results = []
        for batch_idx, batch in enumerate(tqdm.tqdm(dataloader, desc="Processing batches")):
            if len(batch) < 2:
            ...
            all_results.extend(self.process_results(inputs, queries, responses, batch_idx))
    ...
    def process_results(self, inputs, queries, outputs, batch_idx):
        ...
        for i, (input, query) in enumerate(zip(inputs, queries)):
            ...
            for _, (seq, seq_scores) in enumerate(zip(batch_sequences, zip(*batch_scores))):
                ...
                top_tokens = [self.tokenizer.decode([idx.item()]) for idx in top_indices[-1]]
